...
def log_validation_recon_decoder(unet, controlnet, reconstruction_decoder, tokenizer, text_encoder, captured_features, args, accelerator, weight_dtype, step):
    logger.info("Running validation for Reconstruction Decoder...")
    ...
    with torch.no_grad():
        for cond_image_pil in val_cond_images_pil:
            ...
            # Use the text_encoder's config for hidden_size
            dummy_encoder_hidden_states = torch.zeros(bsz, tokenizer.model_max_length, text_encoder.config.hidden_size, device=accelerator.device, dtype=weight_dtype)
            ...
            # Run the UNet to trigger the hook and get the decoder output
            unet(
                dummy_sample,
                dummy_timestep,
                encoder_hidden_states=dummy_encoder_hidden_states,
            ...
            target_cond_image = transforms.functional.rgb_to_grayscale(cond_image_tensor).to(
                device=reconstructed_cond_image.device, dtype=reconstructed_cond_image.dtype
            )
            
            # Use MSE loss as in main training loop
    ...
    if len(original_images_pil) > 0:
        # Concatenate original and reconstructed images for display
        pil_images = []
        for orig_img, recon_img in zip(original_images_pil, recon_images_pil):
            pil_images.append(orig_img)
            pil_images.append(recon_img)

        grid = image_grid(pil_images, rows=args.num_validation_images, cols=2)
        save_path = Path(args.output_dir) / args.recon_decoder_log_dir_name / f"step_{step}_val_recon.png"
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--initial_log_var_similarity",
        type=float,
        default=0.0,
        help="Initial log variance for the similarity loss (log_var_a).",
    )
    parser.add_argument(
        "--initial_log_var_reconstruction",
        type=float,
        default=0.0,
        help="Initial log variance for the reconstruction loss (log_var_b).",
    )
    parser.add_argument(
        "--loss_black_area",
        action="store_true",
...
def main(args):
    ...
    # Optimizer creation
    log_var_a = torch.nn.Parameter(torch.tensor(args.initial_log_var_similarity, device=accelerator.device))
    log_var_b = torch.nn.Parameter(torch.tensor(args.initial_log_var_reconstruction, device=accelerator.device))

    params_to_optimize = list(controlnet.parameters()) + list(reconstruction_decoder.parameters()) + [log_var_a, log_var_b]
    ...
    # Add recon_loss for best model saving
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                L_reconstruction = F.mse_loss(reconstructed_cond_image, target_cond_image, reduction="none")
                L_reconstruction = (L_reconstruction * mask).sum() / (mask.sum() + 1e-8)

                # --- NEW: Similarity Loss Calculation ---
                # 1. Denoise the prediction to get the predicted image in latent space
                pred_original_sample = noise_scheduler.step(model_pred, timesteps, noisy_latents, return_dict=False)[0]
                # 2. Decode the latents to pixel space
                pred_images = vae.decode(pred_original_sample / vae.config.scaling_factor, return_dict=False)[0]
                # 3. Calculate similarity loss
                L_similarity = image_metrics.calculate_placeholder_similarity(pred_images, batch["pixel_values"])
                
                # --- NEW: Dynamic Loss Weighting ---
                precision_a = torch.exp(-log_var_a)
                loss_a = precision_a * L_similarity + log_var_a

                precision_b = torch.exp(-log_var_b)
                loss_b = precision_b * L_reconstruction + log_var_b

                loss = noise_loss + loss_a + loss_b
                # --- End of New Loss Calculation ---

                accelerator.backward(loss)
                if accelerator.sync_gradients:
                    params_to_clip = list(controlnet.parameters()) + [log_var_a, log_var_b]
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.validation_steps == 0:
                        ...
                        # Call the new log_validation_recon_decoder function
                        avg_val_recon_loss = log_validation_recon_decoder(
                            unet,
                            controlnet,
                            reconstruction_decoder,
                            tokenizer,
                            text_encoder, # Pass text_encoder for its config
                            captured_features,
                            args,
                            accelerator,
                            weight_dtype,
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if avg_val_recon_loss is not None:
                            ...
                            if should_save:
                                # Copy the entire checkpoint folder, which includes both controlnet and reconstruction_decoder
                            ...
                            avg_val_recon_loss = None # Reset for next validation

                        # Save best checkpoints based on other metrics (fr_metrics, db_metrics)
                        if image_logs is not None:
                            for label_key in checkpoint_save_list.keys():
                                if label_key == "last" or label_key == "recon_loss": # Skip recon_loss as it's handled above
                                    continue
            ...
            logs = {
                ...
                "recon_loss": L_reconstruction.detach().item(),
                "similarity_loss": L_similarity.detach().item(),
                "log_var_a": log_var_a.detach().item(),
                "log_var_b": log_var_b.detach().item(),
            }
