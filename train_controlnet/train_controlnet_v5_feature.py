...
class Discriminator(nn.Module):
    def __init__(self, input_channels, hidden_channels=64):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(input_channels, hidden_channels, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(hidden_channels, hidden_channels * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(hidden_channels * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(hidden_channels * 2, 1, 4, 2, 1, bias=False),
            nn.Sigmoid(),
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet, discriminator):
                ...
                # Apply weights to the outputs of the ControlNet
                weighted_down_block_res_samples = [sample * reshaped_weights for sample in down_block_res_samples]
                weighted_mid_block_res_sample = mid_block_res_sample * reshaped_weights

                # Predict the noise residual using the weighted conditioning
                model_pred = unet(
                    noisy_latents,
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                    down_block_additional_residuals=[
                        sample.to(dtype=weight_dtype) for sample in weighted_down_block_res_samples
                    ],
