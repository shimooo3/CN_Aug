...
from utils import image_grid, get_reward_model, get_reward_loss, label_transform, group_random_crop, save_validation_images
...
def log_validation(
    ...
    try:
        validation_image_paths = val_dataset[image_column]
        validation_condition_paths = val_dataset[conditioning_image_column]
        validation_prompts = val_dataset[caption_column]
    except:
        validation_image_paths = [item[image_column] for item in val_dataset]
        validation_condition_paths = [item[conditioning_image_column] for item in val_dataset]
        validation_prompts = [item[caption_column] for item in val_dataset]

    validation_images = [Image.open(os.path.join(args.validation_data_dir, p)) for p in validation_image_paths]
    validation_conditions = [Image.open(os.path.join(args.validation_data_dir, p)) for p in validation_condition_paths]

    if args.label_column is not None:
        validation_condition_filenames = [os.path.basename(p) for p in validation_condition_paths]
        validation_label_paths = [os.path.join(args.validation_data_dir, args.label_column, fname) for fname in validation_condition_filenames]
        validation_labels = [Image.open(p) for p in validation_label_paths]
    else:
        validation_labels = validation_images  # Fallback
    ...
    for validation_prompt, validation_condition, validation_image, validation_label in zip(validation_prompts, validation_conditions, validation_images, validation_labels):
        if val_dataset is not None:
            ...
            validation_label = validation_label.convert('RGB').resize((512, 512), Image.Resampling.BICUBIC)
        else:
        ...
        if args.task_name == "green_ratio":
            # CIVE
            image_tensor = torch.stack([transforms.ToTensor()(i) for i in images]).to(accelerator.device)
            image_tensor = image_tensor * 255.0
            r, g, b = image_tensor[:, 0, :, :], image_tensor[:, 1, :, :], image_tensor[:, 2, :, :]
            cive = 0.441 * r - 0.811 * g + 0.385 * b + 18.78745
            binary_images = (cive > 0).float()

            # ground truth mask
            mask_tensor = transforms.ToTensor()(validation_label.convert("L")).to(accelerator.device)

            # multiplied
            multiplied_images = binary_images * mask_tensor.squeeze(1)

            # convert to PIL image
            binary_images = [transforms.ToPILImage()(i) for i in binary_images]
            multiplied_images = [transforms.ToPILImage()(i) for i in multiplied_images]
        ...
        image_logs.append({
            ...
            "images": images,
            'ema_images': [],
            "binary_images": binary_images if args.task_name == "green_ratio" else [],
            "ground_truth_masks": [validation_label] * args.num_validation_images if args.task_name == "green_ratio" else [],
            "multiplied_images": multiplied_images if args.task_name == "green_ratio" else [],
        })
    ...
    # MODIFIED: Add a new function to save the images.
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                # Save images to disk
                if args.task_name == "green_ratio" and step > 0 and step % args.checkpointing_steps == 0:
                    save_validation_images(
                        images,
                        validation_condition,
                        log["binary_images"],
                        log["ground_truth_masks"],
                        log["multiplied_images"],
                        step,
                        args.output_dir,
                    )
...
def make_dataset(args, tokenizer, accelerator, data_dir, split='train'):
    ...
    if args.dataset_name is not None:
        ...
        if data_dir is not None:
            dataset = load_dataset(
                "json",
                data_files=os.path.join(data_dir, "metadata.jsonl"),
                cache_dir=args.cache_dir,
                keep_in_memory=args.keep_in_memory,
            )
    ...
    def preprocess_train(examples):
        # Construct full paths from relative paths in metadata and the base data directory
        image_paths = [os.path.join(data_dir, rel_path) for rel_path in examples[image_column]]
        conditioning_paths = [os.path.join(data_dir, rel_path) for rel_path in examples[conditioning_image_column]]
        ...
        if args.conditioning_image_column in ['canny', 'lineart', 'hed']:
            ...
            conditioning_images = [Image.open(p).convert("RGB") for p in conditioning_paths]
            conditioning_images = [conditioning_image_transforms(image) for image in conditioning_images]

        if args.label_column is not None:
            conditioning_filenames = [os.path.basename(p) for p in examples[conditioning_image_column]]
            label_paths = [os.path.join(data_dir, args.label_column, fname) for fname in conditioning_filenames]
            dtype = torch.long
            labels = [torch.tensor(np.asarray(Image.open(p).convert("L")), dtype=dtype).unsqueeze(0) for p in label_paths]
...
def main(args):
    ...
    _, train_dataset = make_dataset(args, tokenizer, accelerator, args.train_data_dir, 'train')
    val_dataset, _ = make_dataset(args, tokenizer, accelerator, args.validation_data_dir, 'train')
    if val_dataset is not None:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                elif args.task_name in ['canny', 'lineart', 'hed', 'green_ratio']:
                    pass
                else:
                ...
                elif args.task_name == 'green_ratio':
                    outputs = reward_model(image.to(accelerator.device), batch["labels"].to(accelerator.device))
                else:
                ...
                elif args.task_name in ['lineart', 'hed', 'green_ratio']:
                    labels = batch["labels"]
                else:
                    labels = batch["labels"]
                ...
                elif args.task_name in ['depth', 'canny', 'lineart', 'hed', 'green_ratio']:
