...
def get_reward_model(task='segmentation', model_path='mmseg::upernet/upernet_r50_4xb4-160k_ade20k-512x512.py'):
    ...
    elif task == 'green_ratio':
        return GreenRatioModel()
    else:
        raise not NotImplementedError("Only support segmentation, canny, depth, and green_ratio for now.")
...
def get_reward_loss(predictions, labels, task='segmentation', **args):
    ...
    if task == 'segmentation':
        ...
        return dice_loss(predictions, labels, **args)
...
def dice_coeff(pred, target, epsilon=1e-6, **args):
    """Dice coefficient for binary segmentation for a batch of images."""
    # Shape of pred, target: (B, C, H, W)
    axes = tuple(range(1, pred.ndim))
    inter = torch.sum(pred * target, dim=axes)
    union = torch.sum(pred, dim=axes) + torch.sum(target, dim=axes)
    dice = (2. * inter + epsilon) / (union + epsilon)
    return dice # Shape: (B,)

def dice_loss(pred, target, **args):
    """Dice loss for binary segmentation. Binarizes inputs first.
    Adds a small penalty for the singular case where both inputs are empty.
    """
    pred = (pred > 0).float()
    target = (target > 0).float()
    
    # Calculate dice loss per item
    loss = 1 - dice_coeff(pred, target, **args)

    # Identify singular cases (both pred and target are empty) and add a small penalty
    axes = tuple(range(1, pred.ndim))
    union = torch.sum(pred, dim=axes) + torch.sum(target, dim=axes)
    loss = torch.where(union == 0, loss + 1e-4, loss)
...
def label_transform(labels, task, dataset_name, **args):
    ...
    elif task in ['canny', 'lineart', 'hed', 'green_ratio']:
...
class GreenRatioModel(nn.Module):
    def __init__(self):
        super(GreenRatioModel, self).__init__()
    ...
    def forward(self, image, mask):
        ...
        cane_day = 0.0237 * r - 0.0408 * g + 0.0050 * b + 0.2563
        rgb_BCEWithLogits = 0.201833*r + -0.208213*g + -0.098413*b + -0.419534
        
        
        # 二値化
        binary_image = ((cane_day > 0) | (cane_night > 0) | (rgb_BCEWithLogits > 0)).float()

        # 正解マスク領域でフィルタリング
        binary_image = binary_image * mask.squeeze(1)
        
        return binary_image.unsqueeze(1)

def save_validation_images(images, validation_condition, binary_images, ground_truth_masks, multiplied_images, step, output_dir):
    """Save validation images to disk."""
    output_path = os.path.join(output_dir, f"validation_step_{step}")
    os.makedirs(output_path, exist_ok=True)

    for i, (image, binary_image, ground_truth_mask, multiplied_image) in enumerate(zip(images, binary_images, ground_truth_masks, multiplied_images)):
        image.save(os.path.join(output_path, f"{i}_generated.png"))
        validation_condition.save(os.path.join(output_path, f"{i}_condition.png"))
        binary_image.save(os.path.join(output_path, f"{i}_binary.png"))
        ground_truth_mask.save(os.path.join(output_path, f"{i}_mask.png"))
        multiplied_image.save(os.path.join(output_path, f"{i}_multiplied.png"))
